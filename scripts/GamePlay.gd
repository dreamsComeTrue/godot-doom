...
func _process(delta: float) -> void:
	...
	for sector in sectors:
		for wall in sector.walls:
			if not wall.two_sided or (wall.two_sided and _is_player_in_front(player_pos, wall)):
				wall.visible = true

				if wall.line_def_type in door_types and wall.front_side:
...
func _is_player_in_front(player_pos : Vector2, wall) -> bool:
	var front = wall.front_sector_pointer
	front.y *= -1
	var back = wall.back_sector_pointer
	back.y *= -1
	var distance1 = abs(player_pos.distance_to(front))
	var distance2 = abs(player_pos.distance_to(back))

	if distance1 < distance2 and wall.front_side:
		return true
	elif distance2 < distance1 and not wall.front_side:
		return true
...
func render_level() -> void:
	var selected_sectors = [38, 100]#, 38, 41]
...
#		if sidedef.sector in selected_sectors:
	...
	for wall1 in walls:
		...
		if found_sector == null:
			found_sector = sector_blueprint.new()
			found_sector.sector_id = wall1.sector
			found_sector.light_level = wall1.light_level
			found_sector.floor_height = wall1.floor_height
			found_sector.ceil_height = wall1.ceil_height
			sectors.append(found_sector)

		for w in walls_to_be_drawn:
			found_sector.walls.append(w)

		var points_set : Array = _triangulate(walls_to_be_drawn, found_sector)
		...
		for points in points_set:
			for idx in range(0, points.size(), 3):
				...
				if wall1.floor_texture != "F_SKY1":
					...
					found_sector.floor_segments.append(floor_segment)
				...
				if wall1.ceil_texture != "F_SKY1":
					...
					found_sector.ceil_segments.append(ceil_segment)
...
#			var geometry = ImmediateGeometry.new()
	...
	#		add_child(geometry)
			...
			var duplicate = null
			for sector_duplicate in sectors:
				for wall_duplicate in sector.walls:
					if wall_duplicate.id == wall.id:
	...
	_compute_sector_pointers()

func _compute_sector_pointers():
	for sector in sectors:
		for wall in sector.walls:
			...
			var scaler : float = 100000.0
			var xdif : float = (mid_point.x - vertex2.x) / scaler
			var ydif : float = (mid_point.y - vertex2.y) / scaler

			var a1 : float = mid_point.x - ydif / 2.0
			var b1 : float = mid_point.y + xdif / 2.0
			var a2 : float = mid_point.x + ydif / 2.0
			var b2 : float = mid_point.y - xdif / 2.0
			var point1 = Vector2(a1, -b1)
			var point2 = Vector2(a2, -b2)

			var right_sector = get_sidedef(get_linedef(wall.id).right_sidedef).sector
			var left_sector = get_sidedef(get_linedef(wall.id).left_sidedef).sector

#			var spr1 = Sprite3D.new()
#			spr1.texture = load("res://icon.png")
#			spr1.translation = Vector3(point1.x, 2, -point1.y)
#			add_child(spr1)
#
#			var spr2 = Sprite3D.new()
#			spr2.texture = load("res://icon.png")
#			spr2.translation = Vector3(point2.x, 2,-point2.y)
#			add_child(spr2)

			for s in sectors:
				if s.sector_id == right_sector and wall.front_side:
					for poly in s.polygon_set:
						if _point_in_poly(poly, point1):
							wall.front_sector_pointer = point1
							wall.back_sector_pointer = point2
						elif _point_in_poly(poly, point2):
							wall.front_sector_pointer = point2
							wall.back_sector_pointer = point1

				if s.sector_id == left_sector and not wall.front_side:
					for poly in s.polygon_set:
						if _point_in_poly(poly, point1):
							wall.front_sector_pointer = point2
							wall.back_sector_pointer = point1
...
func _triangulate(walls_to_be_drawn: Array, sector) -> Array:
